...
public class ResourcesController extends AbstractController {
	...
	public @ResponseBody ResponseResult commitEdit(HttpServletRequest request, Integer id,String name, String resKey, String resUrl, 
		...
		try {
			...
			if(type ==1 && StringUtils.isEmpty(resUrl)) {
			...
			resourcesService.commitEdit(userKey, id, name, resUrl, resKey, parentId, status, type, description);
			return successJson();
		} catch (Exception e) {
			e.printStackTrace();
		}
