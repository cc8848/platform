...
@Repository
public class GameRewardDaoImpl extends EntityMongoDaoImpl<GameReward> implements IGameRewardDao{

	public void insertGameReward(GameReward gameReward){
		 getMongoTemplate().insert(gameReward, "reward");
	}
	public long searchCount(Long serverId, Date startDate,
		...
		query.addCriteria(criteria);
		return getMongoTemplate().count(query, "reward");
	}
	...
	public List<GameReward> searchList(Long serverId, Date startDate,
        ...
        query.skip(start);
        query.limit(limit);
        return getMongoTemplate().find(query, GameReward.class, "reward");
	}
}
