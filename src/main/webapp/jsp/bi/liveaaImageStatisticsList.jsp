...
<html lang="zh-CN">
	...
	<body>
		<c:set var="preUrl" value="liveaaImageStatisticsSearch
									?startTime=${ startTime }
									&endTime=${ endTime }&" />
		<header class="ui-page-header">
			<div class="mini-title">当前位置：用户拍照统计</div>
		</header>
		<article class="container-fluid">
			...
			<table class="table table-hover table-bordered table-condensed">
				...
				<tbody id="topic-list">
					<c:forEach var="liveaaImageStatistics" items="${ liveaaImageStatisticsList }">
					<tr data-id="${ liveaaImageStatistics.id }">
						<td>${ liveaaImageStatistics.date }</td>
						<td>${ liveaaImageStatistics.imageCount }</td>
					</tr>
					</c:forEach>
				</tbody>
			</table>
