...
<body>
<c:set var="preUrl" value="announcementInfoList
	...
	<header class="ui-page-header">
		<div class="mini-title">当前位置：公告列表管理</div>
	</header>
	<article class="container-fluid">
		<form class="form-inline search-form">
			...
			<div class="form-group">
				<a href="<%=basePath%>tool/announcementInfo" class="btn btn-success ml10">添加公告</a>
			</div>
		</form>
		<table class="table table-hover table-bordered table-condensed">
			<thead>
				<tr class="info">
					...
					<th class="min-w80">公告内容</th>
					<th class="min-w50">发送时间</th>
					<th class="min-w50">操作人</th>
				</tr>
			</thead>
			<tbody id="user-list">
				<c:forEach var="announcementInfo" items="${ announcementList }">
				<tr>
				    <td>${ announcementInfo.index }</td>
					<td>${ announcementInfo.name }</td>
					<td>
					<c:if test="${announcementInfo.content.length() > 16 }">${ announcementInfo.content.substring(0, 16)}... </c:if>
					<c:if test="${announcementInfo.content.length() <= 16 }">${ announcementInfo.content} </c:if> 
					</td>
					<td>${ announcementInfo.createTimeStr }</td>
					<td>${ announcementInfo.operator }</td>
				</tr>
				</c:forEach>
			</tbody>
		</table>
