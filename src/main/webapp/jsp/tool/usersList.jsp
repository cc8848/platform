...
<body>
<c:set var="preUrl" value="usersInfoList
							?userId=${ userId }
							&deviceId=${ deviceId }
							&operator=${ operator }
							&mobile=${ mobile }
	...
	<header class="ui-page-header">
		<div class="mini-title">当前位置：用户列表管理</div>
	</header>
	<article class="container-fluid">
		...
		<table class="table table-hover table-bordered table-condensed">
			...
			<tbody id="user-list">
				...
				<tr>
				<td><a href="<%=basePath %>tool/userDetail?userId=${ usersInfo._id }">${ usersInfo._id }</a></td>
					<td>${ usersInfo.deviceid }</td>
					<td>${ usersInfo.mobile }</td>
					<td>${ usersInfo.loginname }</td>
					<td>${ usersInfo.score }</td>
					<td>${ usersInfo.isMobilebindStr }</td>
					<td>${ usersInfo.isBindStr }</td>
					<td>${ usersInfo.createTimeStr }</td> 
				</tr>
				</c:forEach>
			</tbody>
		</table>
