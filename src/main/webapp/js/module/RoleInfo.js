define('module/RoleInfo', ['lib/jquery', 'util/artTemplate', 'module/Dialog', 'module/Validator', 'util/ajaxPromise'], function(require) {
	...
	authorityTmpl = tmpl.compile([
		...
		'<tr>',
		'{{/if}}',
			'<td{{if $value.type === 0}} rowspan="{{$value.leafNum}}"{{/if}}>',
				'<div class="checkbox">',
				...
				'</div>',
			'</td>',
		'{{if $value.type === 1 || $value.leafNum === 0}}',
		'</tr>',
		'{{/if}}',
		// 利用空判断来实现变量赋值
					...
					content: [
						'<div class="table-scroll">',
							'<table class="table table-bordered table-middle">',
								'<thead>',
									'<tr>',
										'<th class="min-w200">一级目录</th>',
										'<th class="min-w200">二级目录</th>',
										'<th class="min-w200">三级目录</th>',
										'<th class="min-w200">四级目录</th>',
									'</tr>',
								'</thead>',
