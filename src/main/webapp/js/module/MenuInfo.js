define('module/MenuInfo', ['lib/jquery', 'util/artTemplate', 'module/Dialog', 'module/Validator', 'util/ajaxPromise'], function(require) {
	 ...
	 * @param {Boolean} type 菜单类型 0 目录 1菜单
		...
		'<div class="form-horizontal">',
			...
			'<div class="form-group">',
				'<div class="col-sm-offset-2 col-sm-10">',
					'<div class="checkbox">',
						'<label><input class="menu-type" type="checkbox" {{if type}}checked{{/if}}>&nbsp;置为菜单</label>',
					'</div>',
				'</div>',
			'</div>',
			'<div class="form-group">',
			...
			loadMenu().then(function(menuData) {
				dialog.show({
					...
					content: [
						'<div class="form-horizontal">',
							...
							'<div class="form-group">',
								'<div class="col-sm-offset-2 col-sm-10">',
									'<div class="checkbox">',
										'<label><input class="menu-type" type="checkbox">&nbsp;置为菜单</label>',
									'</div>',
								'</div>',
							'</div>',
							'<div class="form-group">',
					...
					confirm: function() {
						...
						data.type = container.find('.menu-type').prop('checked') ? 1 : 0;
